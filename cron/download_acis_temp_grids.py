...
date_fmt = 'last_obs = %(last_obs_date)s, fcast_start = %(fcast_start_date)s, '
date_fmt += 'fcast_end = %(fcast_end_date)s, last_valid = %(last_valid_date)s'

debug = options.debug
dev_mode = options.dev_mode
update_forecast = options.update_forecast
verbose = options.verbose or debug
print '\ndownload_acis_temp_grids.py', args
...
#static_factory = NdfdStaticFileFactory(None)
#if dev_mode: static_factory.useDirpathsForMode('dev')
#reader = static_factory.ndfdStaticFileReader(source, region)
#static_shape = reader.datasetShape('lon')
#print static_shape
#reader.close()
#del reader, static_factory
...
date_attrs = manager.dateAttributes('temps.maxt')
...
if end_date == start_date:
    msg = 'downloading %s temps for %s'
    print msg % (source.tag, start_date.strftime('%B %d, %Y'))  
    end_date = None
else:
    msg = 'downloading %s temps for %s thru %s'
...
if debug:
    ...
    print '   temp file :', manager.filepath
...
data = factory.getAcisGridData(int(acis_grid), 'mint,maxt', start_date,
                               end_date, False, bbox=region.data, 
                               verbose=verbose, debug=debug)
print 'mint', data['mint'].shape
if debug: print '       -999 :', N.where(data['mint'] == -999)
data['mint'][N.where(data['mint'] == -999)] = -32768
if debug: print '     -32768 :', [N.where(data['mint'] == -32768)]

print 'maxt', data['maxt'].shape
if debug: print '       -999 :', N.where(data['maxt'] == -999)
data['maxt'][N.where(data['maxt'] == -999)] = -32768
if debug: print '     -32768 :', [N.where(data['maxt'] == -32768)]

maxt = data['maxt']
if len(N.where(N.isnan(maxt))[0]) == N.prod(maxt.shape):
    print 'No valid data returned from ACIS, execution terminated.'
    exit()

mint = data['mint']

if debug: print 'updating "temps" group'
manager.open('a')
manager.updateTempGroup(start_date, mint, maxt, source.tag, forecast=False)
manager.open('a')
date_attrs = manager.dateAttributes('temps.maxt')
manager.close()
